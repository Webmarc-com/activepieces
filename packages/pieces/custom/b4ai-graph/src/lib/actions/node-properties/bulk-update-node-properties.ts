...
export const bulkUpdateNodeProperties = createAction({
  auth: graphAuth,
  name: 'bulk_update_node_properties',
  displayName: 'Bulk Update Node Properties',
  description: 'Update multiple node properties at once',
  props: {
  ...
  async run(context) {
    ...
    return {
      ...
      message: `Successfully updated ${response.updated} node properties${response.errors.length > 0 ? ` with ${response.errors.length} errors` : ''}`,
    };
  },
});
