...
export const bulkUpdateRelationshipTypes = createAction({
  auth: graphAuth,
  name: 'bulk_update_relationship_types',
  displayName: 'Bulk Update Relationship Types',
  description: 'Update multiple relationship types at once',
  props: {
  ...
  async run(context) {
    ...
    return {
      ...
      message: `Successfully updated ${response.updated} relationship types${response.errors.length > 0 ? ` with ${response.errors.length} errors` : ''}`,
    };
  },
});
