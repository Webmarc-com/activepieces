...
import { uploadApiCall } from './client';

export const b4aiUtilitiesAuth = PieceAuth.CustomAuth({
  description: `**B4AI Utilities API Authentication**
...
Note: Both API Key and API Secret are required for authentication.
  ...
  validate: async ({ auth }) => {
    try {
      // Validate the credentials by listing files
      await uploadApiCall({
